...
export function registerUpdateTool(server) {
    server.tool("mural_update_board", "Update a Mural board. Accepts either a desired-state spec (computes diff automatically) or explicit operations (create/update/delete). Use dry_run to preview changes.", {
        ...
        let diff;
        if (isExplicitOps) {
            diff = parseExplicitOps(parsed);
        }
        else {
            const serverSpec = await extractBoard(client, muralId);
            diff = computeDiff(serverSpec, parsed);
