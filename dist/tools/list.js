...
export function registerListTool(server) {
    server.tool("mural_list_boards", "List or search Mural boards in a workspace or room. Use 'title' to filter by name (case-insensitive substring match).", {
        ...
        if (!workspaceId && !roomId) {
            ...
            const resp = (await client.request("GET", "/workspaces"));
        ...
        while (nextPath) {
            ...
            const murals = unwrapList(resp);
            allMurals = allMurals.concat(murals);
            nextPath = resp.next || null;
        }
        if (title) {
            const search = title.toLowerCase();
            allMurals = allMurals.filter((m) => m.title?.toLowerCase().includes(search));
        }
