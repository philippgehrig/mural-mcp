...
export function parseExplicitOps(input) {
    ...
    if (input.create) {
        for (const [key, widgets] of Object.entries(input.create)) {
            ...
            for (const w of widgets) {
                if (w.id) {
                    throw new Error(`Widget in create must not have an id (got "${w.id}" in ${specKey})`);
                }
            }
    ...
    if (input.update) {
        for (const [key, widgets] of Object.entries(input.update)) {
            ...
            for (const w of widgets) {
                if (!w.id) {
                    throw new Error(`Widget in update must have an id (missing in ${specKey})`);
                }
                const { id, ...fields } = w;
                if (Object.keys(fields).length === 0) {
                    throw new Error(`Widget in update must have at least one field besides id (widget "${id}" in ${specKey})`);
                }
