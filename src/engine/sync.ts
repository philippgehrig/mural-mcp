import type { BoardDiff, SyncReport, SpecKey, Widget } from "../api/types.js";
...
export async function applyDiff(
  ...
  for (const widgetId of diff.deletes) {
    try {
      ...
      report.errors.push(`Delete ${widgetId}: ${(err as Error).message}`);
    }
  }

  // Step 2: Creates — areas first (need their IDs for parentId resolution)
  const areaIdMap = new Map<number, string>(); // index → server ID
  ...
  for (const group of areaCreates) {
    try {
      ...
      for (let i = 0; i < created.length; i++) {
        areaIdMap.set(i, created[i].id as string);
      }
      report.created += created.length;
    } catch (err) {
      report.errors.push(`Create areas: ${(err as Error).message}`);
    }
  }
  ...
  for (const group of otherCreates) {
    try {
      ...
      report.errors.push(`Create ${group.specKey}: ${(err as Error).message}`);
    }
  }
  ...
  for (const update of diff.updates) {
    try {
      const fields = normalizeWidgetHtml(update.fields as Widget);
      await client.updateWidget(muralId, update.widgetId, update.specKey, fields);
      report.updated++;
    } catch (err) {
      report.errors.push(`Update ${update.widgetId}: ${(err as Error).message}`);
    }
  }
