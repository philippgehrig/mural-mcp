import type { Widget, BoardSpec, BoardDiff, SpecKey } from "../api/types.js";
import { SPEC_KEYS, SERVER_ONLY_FIELDS } from "../api/types.js";

function stripMetadata(widget: Widget): Widget {
  const result: Widget = {};
...
function changedFields(desired: Widget, server: Widget): Partial<Widget> {
  ...
  for (const [key, value] of Object.entries(sd)) {
    ...
    if (key === "parentId" && typeof value === "number") continue;
...
function posMatch(a: Widget, b: Widget, tolerance: number): boolean {
  const ax = a.x, ay = a.y, bx = b.x, by = b.y;
  if (ax == null || ay == null || bx == null || by == null) return false;
  return (
    Math.abs(Number(ax) - Number(bx)) <= tolerance &&
    Math.abs(Number(ay) - Number(by)) <= tolerance
  );
}

export function computeDiff(
  serverWidgets: BoardSpec,
  desiredWidgets: BoardSpec,
  ...
  for (const specKey of SPEC_KEYS) {
    ...
    const serverById = new Map<string, Widget>();
    const serverUnmatched: Widget[] = [];
    ...
    for (const dw of desiredList) {
      ...
      if (did) {
        ...
        if (!sw) {
          throw new Error(
            `Stale widget ID "${did}" in ${specKey}: not found on server. ` +
        ...
        if (match) {
          if (matchIdx >= 0) serverUnmatched.splice(matchIdx, 1);
          const mid = match.id;
          if (mid) {
    ...
    for (const sw of serverUnmatched) {
      if (sw.id) diff.deletes.push(sw.id);
    }
  }
